...
const parseFileStdout = (output, filePath, editor) => {
  const messages = [];
...
const parseCompileStdout = (output, filePath) => {
  const messages = [];
...
module.exports = {
  activate() {
    ...
    const installLinterReekDeps = () => {
      ...
      if (!atom.inSpecMode()) {
        require('atom-package-deps').install('inter-scalac');
      }
      loadDeps();
    };
  ...
  provideLinter() {
    return {
      ...
      lint: async (editor) => {
        ...
        return parseFileStdout(output.stderr, filePath, editor)
          .concat(parseCompileStdout(output.stderr, filePath));
      },
    };
  },
};
