  ...
  activate: () => {
    ...
    this.subscriptions.add(
      atom.config.observe('linter-scalac.scalacExecutablePath',
        scalacExecutablePath => this.scalacExecutablePath = scalacExecutablePath));

    this.subscriptions.add(
      atom.config.observe('linter-scalac.scalacOptions',
        scalacOptions => this.scalacOptions = scalacOptions));

    this.subscriptions.add(
      atom.config.observe('linter-scalac.compileClassesToClasspath',
        compileClassesToClasspath => {
          this.compileClassesToClasspath = compileClassesToClasspath;
          if (!compileClassesToClasspath)
            atom.config.set('linter-scalac.compileAllClassesOnLint', false);
        }));

    this.subscriptions.add(
      atom.config.observe('linter-scalac.compileAllClassesOnLint',
        compileAllClassesOnLint => {
          this.compileAllClassesOnLint = compileAllClassesOnLint;
      ...
      lint: (textEditor) => {
        ...
        if (helpers.find(projectPath, '.classpath')) {
          ...
          if (this.compileAllClassesOnLint) {
            paths = glob.sync(path.join("**", "*.scala"), {cwd: projectPath})
        ...
        return helpers.exec(this.scalacExecutablePath, args, {stream: 'stderr'})
          .then(output =>
